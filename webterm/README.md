...
要送出上表沒有的組合（例如 `Ctrl+X`、`Ctrl+D`）時：

1. 按一下 **Ctrl** —— 按鈕變橘色，旁邊出現「Ctrl 已按下 —— 請再敲一個鍵」。
...
- 再按一次 **Ctrl** 可以取消等待。
- 等待中若改按其他快捷鍵（例如 Tab），該鍵會照原樣送出，Ctrl 等待狀態同時解除。

### Break 鍵（目前停用）

serial break 是**線路狀態訊號**（把 TX 線拉低超過一個字元時間），**不是一個字元**，
所以沒辦法「送某幾個位元組」達成。裝置端目前的藍牙通道是純資料流、
沒有頻外指令機制，因此這顆按鈕停用並標示原因，而不是做成按了沒反應的假按鈕。
...
| **Break** 按鈕按不下去 | 目前藍牙通道做不到 serial break，屬預期行為。改用管理介面的網頁終端，或用 RFC 2217 軟體連 `<裝置 IP>:4001`（見上方「Break 鍵」） |
